...
const BlogTTS: React.FC<BlogTTSProps> = ({
  content,
  title,
  excerpt,
  className = "",
  ...
  const isOnlineVoice = useCallback((voice: SpeechSynthesisVoice) => {
    ...
    if (isMobile) {
      ...
      // Only filter out obviously problematic voices on mobile
      return (
        name.includes("cloud") ||
    ...
    // Desktop: more restrictive filtering
    const name = voice.name.toLowerCase();
    return (
  ...
  const findBestVoice = useCallback(() => {
    ...
    console.log(
      "BlogTTS: Finding best voice from",
      voices.length,
      "available voices"
    );
    ...
    if (isMobile) {
      // On mobile, be more permissive with voice selection
      let englishVoices = voices.filter(
        (voice) => voice.lang.startsWith("en") && !isOnlineVoice(voice)
      );

      // If no English voices after filtering, try all English voices
      if (englishVoices.length === 0) {
        englishVoices = voices.filter((voice) => voice.lang.startsWith("en"));
      }

      // Mobile priority order (include Google voices for Android)
      ...
      for (const preferred of mobilePreferredVoices) {
        ...
        if (voice) {
          console.log("BlogTTS: Selected mobile voice:", voice.name);
          return voice;
        }
      }

      // Fallback to first English voice on mobile
      if (englishVoices.length > 0) {
        console.log(
          "BlogTTS: Using fallback mobile voice:",
          englishVoices[0].name
        );
        return englishVoices[0];
      }
    } else {
      // Desktop voice selection (original logic)
      ...
      for (const preferred of preferredVoices) {
        ...
        if (voice) {
          console.log("BlogTTS: Selected desktop voice:", voice.name);
          return voice;
        }
      }
      ...
      if (englishVoices.length > 0) {
        console.log(
          "BlogTTS: Using fallback desktop voice:",
          englishVoices[0].name
        );
        return englishVoices[0];
      }
    ...
    console.log("BlogTTS: Using final fallback voice:", fallbackVoice?.name);
            ...
            >
              {isPlaying && !isPaused ? (
                ...
                >
                  <path
                    fillRule="evenodd"
                    d="M6 4a1 1 0 011 1v10a1 1 0 11-2 0V5a1 1 0 011-1zm4 0a1 1 0 011 1v10a1 1 0 11-2 0V5a1 1 0 011-1z"
                    clipRule="evenodd"
                  />
                </svg>
              ) : (
          ...
          <div className="flex items-center space-x-2">
            <div className="text-right text-xs text-neutral-500 dark:text-neutral-400 hidden sm:block">
              <p className="truncate max-w-[80px]">
                {selectedVoice?.name?.split(" ")[0] || "No voice"}
              </p>
                ...
                >
                  ...
                  {(() => {
                    let filtered = voices.filter(
                      (v) => v.lang.startsWith("en") && !isOnlineVoice(v)
                    );
